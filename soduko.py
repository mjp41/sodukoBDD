...
def build_all_false(*a):
  if len(a) == 0:
    return bdd.true
  return bdd.And(bdd.Not(bdd.Prop(a[0])), build_all_false(*a[1:]))

def build_all_true(*a):
  if len(a) == 0:
    return bdd.true
  return bdd.And(bdd.Prop(a[0]), build_all_true(*a[1:]))
...
def build_precisely_one(*a):
  ...
  if len(a) == 1:
    return bdd.Prop(a[0])
  lhs = bdd.And(bdd.Prop(a[0]), build_all_false(*a[1:]))
  rhs = bdd.And(bdd.Not(bdd.Prop(a[0])), build_precisely_one(*a[1:]))
...
def solve(lines, block_row, block_column):
  ...
  for i, row in enumerate(lines):
    for j, c in enumerate(row):
      if c != " ":
        base = bdd.And(base, bdd.Prop(cell(i, j, int(c))))
